...
_context.invoke("Nittro.Extras.DropZone.Bridges.DropZoneDI",function(){var DropZoneExtension=_context.extend("Nittro.DI.BuilderExtension",function(containerBuilder,config){DropZoneExtension.Super.call(this,containerBuilder,config)},{load:function(){this._getContainerBuilder().addFactory("dropZone","Nittro.Extras.DropZone.DropZone::create()")}});_context.register(DropZoneExtension,"DropZoneExtension")});
_context.invoke(function(Nittro){var builder=new Nittro.DI.ContainerBuilder({params:{},extensions:{ajax:"Nittro.Ajax.Bridges.AjaxDI.AjaxExtension()",forms:"Nittro.Forms.Bridges.FormsDI.FormsExtension()",page:"Nittro.Page.Bridges.PageDI.PageExtension()",flashes:"Nittro.Flashes.Bridges.FlashesDI.FlashesExtension()",checklist:"Nittro.Extras.CheckList.Bridges.CheckListDI.CheckListExtension()",dialogs:"Nittro.Extras.Dialogs.Bridges.DialogsDI.DialogsExtension()",confirm:"Nittro.Extras.Confirm.Bridges.ConfirmDI.ConfirmExtension()",dropzone:"Nittro.Extras.DropZone.Bridges.DropZoneDI.DropZoneExtension()"},services:{},factories:{}});this.di=builder.createContainer();this.di.runServices()});
window._stack||(window._stack=[]);(function(stack){function exec(f){if(typeof f==="function"){_context.invoke(f)}else if(typeof f==="object"&&typeof f.load!=="undefined"){var q=_context.load.apply(_context,f.load);if(typeof f.then==="function"){q.then(f.then)}else if(f.then&&f.then instanceof Array){q.then.apply(_context,f.then)}}else{_context.invoke.apply(_context,f)}}while(stack.length){exec(stack.shift())}stack.push=function(){for(var i=0;i<arguments.length;i++){exec(arguments[i])}}})(window._stack);
//# sourceMappingURL=nittro.min.map
